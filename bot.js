...
client.on("guildMemberAdd", member => {  
    const kanal = member.guild.channels.cache.find(r => r.id === "787736311313727588");
    const register = "<@&787736310990503954>"
...
client.on('guildMemberAdd', loz => {
  const emoji = client.emojis.get('KalpGif');
  const mesaj = new Discord.RichEmbed()
    .setTitle(`HOSGELDIN`)
    .setDescription(`${emoji} ${loz} Hoşgeldin Seni Arazmıda Görmek Ne Güzel <3`)
  loz.guild.channels.get(`787736311313727588`).send(mesaj)
  loz.guild.channels.get(`787736311313727588`).send({
    file: "https://i.hizliresim.com/K1kym1.gif"
  })
});


client.on("ready", () => {
  client.channels.cache.get("787736311313727589").join();   
}) 
