...
exports.run = async (client, message, args) => {
 ...
 if(!['787736310990503954'].some(role => message.member.roles.cache.get(role)) && !message.member.hasPermission('ADMINISTRATOR')) return message.reply(`Bu Komut İçin Yetkiniz Bulunmamaktadır.`) 
  
let tag = "☸ "
const kayıtlı = message.guild.roles.cache.find(r => r.id === '787736310990503946')
const kayıtsız = message.guild.roles.cache.find(r => r.id === '787736310940565525')
...
.setFooter(`☸ Polaris`)
.setColor('BLUE')
client.channels.cache.get('787736311313727588').send(embed)
